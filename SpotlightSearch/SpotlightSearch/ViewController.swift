...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        attributeSet.title = itemDic["name"] as? String
        attributeSet.contentDescription = itemDic["description"] as? String
        let imageData = UIImageJPEGRepresentation((itemDic["cover"] as? UIImage)!, 0.7)
        attributeSet.thumbnailData = imageData
        
        let searchItem = CSSearchableItem(uniqueIdentifier: "\(indexPath.row)", domainIdentifier: "nimomeng", attributeSet: attributeSet)
